...
function initMap() {
    ...
    L.marker([UNILAG_LAT, UNILAG_LNG], { icon: unilagIcon }).addTo(map);
        ...
        createMarker: (i, wp) => {
            ...
            if (i === 1) {
                return L.marker(wp.latLng, {
                    icon: L.divIcon({
                        className: '',
                        html: `<div style="background:#c62828;width:14px;height:14px;border-radius:50%;border:3px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,0.3);"></div>`,
                        iconSize: [14, 14],
                        iconAnchor: [7, 7]
                    })
                });
